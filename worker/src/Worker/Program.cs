...
namespace Worker
{
    public class Program
    {
        public static int Main(string[] args)
        {
            ...
            {
                var pghost =
                    Environment.GetEnvironmentVariable("PGHOST") ?? "localhost";
                var pguser =
                ...
                var pgConnString =
                    $"Server={pghost};Username={pguser};Password={pgpassword};";
                ...
                while (true)
                {
                    ...
                    if (json != null)
                    {
                        var vote =
                            JsonConvert
                                .DeserializeAnonymousType(json, definition);
                        Console
                            .WriteLine($"Processing vote for '{vote.vote}' by '{
                                vote.voter_id}'");
                        ...
                        if (
                            !pgsql
                                .State
                                .Equals(System.Data.ConnectionState.Open)
                        )
                        {
        ...
        OpenDbConnection(string connectionString)
        {
            ...
            using (
                var cmd =
                    new NpgsqlCommand(@"INSERT INTO votes (id, vote) VALUES
                                    ('fake1', 'a'),
                                    ('fake2', 'a'),
                                    ('fake3', 'a'),
                                    ('fake4', 'a'),
        ...
        OpenRedisConnection(string hostname)
        {
